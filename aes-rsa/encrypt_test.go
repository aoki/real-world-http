...
func prepareRSA() (sourceDate, label []byte, privateKey *rsa.PrivateKey) {
	sourceDate = make([]byte, 128)
	label = []byte("")
	io.ReadFull(rand.Reader, sourceDate)
...
func BenchmarkRSAEncryption(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rsa.EncryptOAEP(md5hash, rand.Reader, publickKey, sourceData, label)
	}
}
